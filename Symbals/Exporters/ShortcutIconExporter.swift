...
struct ShortcutIconExporter: Exporter {
    ...
    func data(for symbol: Symbol, weight: UIImage.SymbolWeight, scale: UIImage.SymbolScale, in font: CTFont, viewScale: CGFloat) -> Data {
        ...
        format.opaque = false
        let drawingSize = CGSize(width: size.width + 16, height: size.height + 16)
        adjustX += 8
        adjustY += 8
        let renderer = UIGraphicsImageRenderer(size: drawingSize, format: format)
        
        let imageData = renderer.pngData { (context) in
            context.cgContext.translateBy(x: 0.0, y: drawingSize.height)
